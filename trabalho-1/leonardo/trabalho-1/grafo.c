	...
	for(Agnode_t *v=agfstnode(ag); v; v=agnxtnode(ag,v)){
		...
		if(!g->direcionado){
			for (Agedge_t *a=agfstedge(g,v); a; a=agnxtedge(g,a,v)){
				aresta edge= malloc (sizeof(struct aresta);
				edge->origem= vert;
				char *w = agget(a,(char*)"peso");
                    edge->peso = w ? strtod(w,NULL) : 0.0;
				//edge->destino= não sei oq por aqui ainda
				insere_lista(edge,vert->vizinhos_entrada);
				insere_lista(edge, vert->vizinhos_saida);
